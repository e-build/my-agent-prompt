...
| `pi/extensions/` | pi 전용 extension 소스 (외부 참고 + 사용자 개발) |
...
| `pi/extensions/cliproxyapi-sync.ts` | `~/.pi/agent/extensions/cliproxyapi-sync.ts` | `bash pi/install.sh --restore`로 심링크 설치 (cp 불가) |
...
# pi extensions (cliproxyapi-sync symlink via install.sh --restore)
bash $REPO/pi/install.sh --restore
# 제거: 심링크 수동 삭제
rm -f ~/.pi/agent/extensions/cliproxyapi-sync.ts
```
