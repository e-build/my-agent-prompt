...
| `opencode/commands/*.md` | `~/.config/opencode/commands/` | thin wrapper, `!\`cat bodies/<name>.md\`` 로 본문 주입 |
| `skills/{shopl,super,tool,dev}/*/` | `~/.config/opencode/skills/{skill}/` | OpenCode에서 재사용하는 Agent Skills (카테고리별 구성) |
...
| 저장소 경로 | Pi 대상 경로 | 비고 |
|-------------|-------------|------|
| `pi/commands/*.md` | `~/.pi/agent/prompts/` | 저장소에서는 `commands/`로 관리하지만 Pi는 `prompts/`로 로드 |
| `bodies/{me-breakdown-to-two-way,me-flip-action-plan,me-flip-think,me-ladder-compare,me-ladder-debug-me,me-ladder-explain,me-ladder-find-gaps,me-ladder-quiz-me,me-ladder-roadmap,me-ladder-show-code,me-ladder-summarize,zz-workflow-design-system,zz-workflow-init,zz-workflow-new}.md` | (공유 본문 — 로컬 심링크 없음) | Pi `pi/commands/*.md` wrapper가 `bodies/`를 참조 |
| `skills/{shopl,super,tool,dev}/*/` | `~/.pi/agent/skills/{skill}/` | Pi에서 관리하는 Agent Skills (카테고리별 구성) |
