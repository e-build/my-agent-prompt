...
8. 학습자는 결과 화면에서 개념 학습에서 더 깊게 다루고 싶은 문항을 `pinpoint`하고 문항별 comment를 남길 수 있다.
9. 학습자가 브라우저에서 “Pi에서 개념 학습 시작” 버튼을 누르면 `# DIAGNOSIS_RESULTS_REVIEWED` 신호가 들어온다. **이 신호를 받은 뒤에야** diagnosis.md의 취약 분야와 학습자가 강조한 pinpoint를 반영해 개념 학습을 시작한다.
...
이제 diagnosis.md의 결과와 학습자가 강조한 pinpoint를 바탕으로 개념 학습을 시작하세요.
```

이 신호를 받으면:
1. 학습자가 강조한 pinpoint가 있으면 가장 먼저 다룬다. 점수상 약점보다 사용자가 직접 콕 집은 문항이 우선이다.
2. 그 다음 diagnosis.md의 약점/권장 학습 깊이를 기준 삼아 개념 학습을 진행한다.
3. 오답/부분정답 문항을 우선 커버한다.
4. 정답이지만 pinpoint된 문항은 "감으로 맞춘 문항"일 수 있으므로 짧게 확인하고 넘어간다.
5. 학습자가 본 학습 범위를 벗어나지 않도록 범위를 한정한다.
...
2. 챕터 README와 학습 목표를 읽고 `DiagnosisQuestionSet` JSON을 구성한다. 이때 반드시 문항 구성 규칙을 지켜 **최소 10문항, 객관식 약 70%, 주관식 약 20%, 서술형 약 10%**로 만든다.
...
4. tool이 HTML 생성 + 브라우저 자동 open을 끝내면, 학습자에게 안내한다:
   - "브라우저를 열었습니다. 답안을 작성하고 제출하세요. 채점 결과를 확인한 뒤 더 깊게 배울 문항을 pinpoint할 수 있습니다. 마지막에 'Pi에서 개념 학습 시작'을 누르면 이어서 진행합니다."
