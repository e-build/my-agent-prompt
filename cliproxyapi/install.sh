...
#
# 동작(동기화 모드):
#   bash cliproxyapi/install.sh --sync-from <실제config.yaml>
#     → 실제 config에서 키만 빨간색(플레이스홀더) 처리해
#       cliproxyapi/config.template.yaml 을 갱신한다. (드리프트 방지)
#
# 사용법:
#   bash cliproxyapi/install.sh                                     # 기본 $HOME/cliproxyapi
#   CLIPROXY_DIR=/opt/cpa bash cliproxyapi/install.sh               # 대상 디렉토리 변경
#   CLIPROXY_SECRET_KEY=... CLIPROXY_ZAI_API_KEY=... \
#     CLIPROXY_DEEPSEEK_API_KEY=... bash cliproxyapi/install.sh     # 키 주입(bash 네이티브, 의존성 없음)
#   bash cliproxyapi/install.sh --from ~/cliproxyapi/config.yaml    # 기존 config 통째로 복사
#   bash cliproxyapi/install.sh --sync-from ~/cliproxyapi/config.yaml  # 실제→템플릿 갱신
#   bash cliproxyapi/install.sh --no-compose                        # compose 실행 없이 설치만
#
...
HDR
  ...
  echo "==> 완료. 변경분 확인 후 커밋: git diff cliproxyapi/config.template.yaml"
  exit 0
fi
