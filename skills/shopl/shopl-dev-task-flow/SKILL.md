...
2. 작은 분류마다 **별도 unit 파일**을 생성한다 (`task-workflow/<큰분류키>/<NN>-<slug>.md`). 큰 분류 파일의 본문에는 작은 분류 실행 로그를 담지 않는다.
3. 작은 분류의 접두사 태그(`[골격]`, `[공통/대상자]`, `[API]`, `[Excel]` 등)는 unit 파일명과 헤더에 보존하여 작업 성격을 구분한다.
4. 티켓 Description에는 큰 분류 제목만 기재한다. 세부 작업은 유동적이므로 티켓에 기록하지 않고 unit 파일이 정본이다.
