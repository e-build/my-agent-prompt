...
- 소스 문서 내 `## 15. 구현 큰 분류` 섹션을 기본 소스로 사용한다.
- 해당 섹션의 `### 큰 분류 N. <작업명>` 각 항목이 **티켓 1개**가 된다.
- `공통 컴포넌트 매핑표`, `API Draft 인계 항목`, `Traceability Matrix` 도 함께 읽어 연관 관계를 파악한다.
- **소스는 큰 분류까지만 제공한다.** 작은 분류는 소스에 없으며, 각 큰 분류 진입 시 Orientation에서 코드/현황/API Draft/정책 조건을 다시 읽어 생성한다(아래 `작은 분류 생성` 참조).
...
작은 분류는 소스(백엔드 분해 문서)에 존재하지 않는다. **각 큰 분류의 Orientation에서 생성**한다. 분해 문서 작성 시점과 실제 구현 시점 사이에 코드/현황이 달라지므로, 큰 분류 진입 시점에 코드를 다시 읽고 작은 분류를 정의하는 쪽이 정확하다.

생성 시점:
- 큰 분류 Orientation(아래 `작업별 절차` 4단계)에서 코드/현황/API Draft/정책 조건/공통 컴포넌트 매핑표를 읽고 작은 분류 후보를 도출한다.
- 도출 결과를 큰 분류 `index.md`의 `Unit Index`에 기록하고, 방향 확인 게이트에서 승인받는다.

생성 원칙:
1. 작은 분류마다 **별도 unit 파일**을 생성한다 (`task-workflow/<큰분류키>/<NN>-<slug>.md`). 큰 분류 파일의 본문에는 작은 분류 실행 로그를 담지 않는다.
2. 접두사 태그(`[골격]`, `[공통/대상자]`, `[API]`, `[Excel]`, `[테스트]` 등)를 unit 파일명과 헤더에 붙여 작업 성격을 구분한다.
3. API/비즈니스 로직/권한/정책 분기를 다루는 큰 분류에는 **반드시 `[테스트]` 작은 분류를 포함**하고, 정렬 원칙(아래)에 따라 `[테스트] → [기능 개발]`로 인접 배치한다.
4. 티켓 Description에는 큰 분류 제목만 기재한다. 작은 분류는 유동적이므로 티켓에 기록하지 않고 unit 파일이 정본이다.

대표 작은 분류 유형: `[골격]` 레이어별 스켈레톤 / 분석·조사 / DTO 설계 / 도메인·정책 설계 / `[테스트]` 테스트 케이스 논의·코드 작성 / 구현. API 엔드포인트를 신규·변경하는 큰 분류의 첫 작은 분류는 `[골격]` 레이어별 스켈레톤 클래스 생성(빈 구현/시그니처). 엔드포인트가 없는 큰 분류(DTO-only/도메인-only/Excel-only 등)는 `[골격]` 단계에서 제외한다.

### 작은 분류 정렬 및 테스트 원칙

작은 분류의 순서와 테스트 정의는 아래 원칙을 따른다. Orientation에서 작은 분류를 생성할 때 `실행 순서`에, 그리고 각 테스트 unit 파일의 계획에 이 원칙이 반영된다.

#### 실행 순서 (test-first ordering)

- 기능 개발 작은 분류가 있으면, **그 기능을 검증하는 테스트 케이스 작은 분류를 직전에 배치**한다.
- 단위 쌍이 `[테스트] → [기능 개발]` 순으로 인접 정렬된다. 같은 기능에 단위 테스트와 통합 테스트가 모두 있으면 단위 테스트를 먼저 둔다.
- 골격/공통 컴포넌트/설정/마이그레이션 등 비기능 작은 분류는 테스트-기능 쌍보다 상단에 둔다.
...
| 순번 | 작은 분류                        | 상태 |
|------|----------------------------------|------|
| 1    | [공통] 엔티티/리포지토리 골격     | 대기 |
| 2    | [테스트] 요약 로직 단위 테스트    | 대기 |
| 3    | [기능] 주간 요약 집계 로직 구현    | 대기 |
| 4    | [테스트] Facade 통합 테스트(happy) | 대기 |
| 5    | [기능] Facade API 구현            | 대기 |
```
...
- 기획서·정책서에서 **분기되는 비즈니스 로직**을 모두 식별해 커버한다 (조건 분기, 권한/상태 전이, enum 가능값별 처리).
- 정책서의 **엣지 케이스**를 누락 없이 포함한다 (경계값, 예외 케이스, 빈 값/초과/권한 거부 등).
- **Facade 계층은 happy path 기준 통합 테스트**로 작성한다. Facade 내부 분기 로직은 도메인/서비스 단위 테스트로 별도 커버한다.
- 구체적인 테스트 케이스 표와 **구현 전 사용자 승인 게이트**는 `shopl-dev-task-flow-unit`의 Test Case Review에서 필수로 수행한다(이 단계는 정책/커버리지 관점만 정의).

#### TDD 실행

- 기능 개발 작은 분류는 **TDD 사이클(RED → GREEN → REFACTOR)** 로 구현한다.
- 테스트 작은 분류가 직전에 테스트를 만들어두므로, 기능 개발 작은 분류 진행 중 **테스트 실행을 적극적으로 돌려** 실패→통과를 확인하며 구현한다.
- 해당 기능 관련 테스트가 전부 통과해야 diff 승인 게이트로 넘어간다. 상세 절차는 `shopl-dev-task-flow-unit` 스킬을 따른다.
...
- 실행 순서: `<작은 분류 순서, 테스트-기능 인접 정렬 원칙 적용 — 작은 분류 정렬 및 테스트 원칙 참조>`
- 공통 결정: `<공통 컴포넌트/데이터 모델/재사용 결정>`
- 주의: 큰 분류 전체 상세 계획을 한 번에 확정하지 않는다. 상세 계획은 `shopl-dev-task-flow-unit`에서 작은 분류별로 unit 파일에 작성한다.
...
5. **추적 인덱스 채우기** — Task Index에 번호/작업명/난이도/작업 키/상태를 모두 채운다. **큰 분류 `index.md`** 도 빈 템플릿으로 미리 생성. (작은 분류 unit 파일은 Orientation에서 생성하므로 여기서 만들지 않는다.)
...
- **다음 실행 unit의 진실의 소스는 큰 분류 `index.md`의 Unit Index `상태` 열 하나다.** 최상위 인덱스의 `최근 메모`는 상태 요약용 힌트일 뿐이며, 여기에 `다음=...` 형태의 다음 작업 지시문을 적지 않는다. 다음 unit 정보는 Unit Index에만 존재해 이중 동기화를 원천 차단한다. (메모와 Unit Index가 불일치하면 Unit Index를 진실로 삼고 메모를 정정한다.)
...
- unit 파일 명명 규칙:
    ...
    - 원문 작은 분류 제목은 unit 파일 헤더 메타데이터에 보존한다.
...
- 큰 분류 파일은 컨테이너 역할만 한다. 작은 분류 기록은 unit 파일에 둔다.
...
- 기획 변경·구두 합의·회의 결정은 `planning-change-log.md`에 PC-ID로 기록하고, 부모 스킬이 중앙에서 관리한다. 상세는 `Planning Change Log 관리` 참조.
...
- 실행 순서:
- 공통 결정:
- 주의: 상세 계획은 `shopl-dev-task-flow-unit` 스킬에서 작은 분류별로 unit 파일에 작성한다.
...
위 표는 **큰 분류 단위**의 상태다. 작은 분류(unit)는 더 촘촘한 전용 상태 모델을 쓴다 (`대기 / 계획 작성 / 승인 대기 / 진행 중 / diff 검토 중 / diff 거절 / 검증 중 / 완료 / 차단`). 상세 정의는 `shopl-dev-task-flow-unit` 스킬을 본다. 작은 분류 상태는 트래커와 동기화하지 않고 로컬 진행 제어 전용이며, 큰 분류 파일의 Unit Index와 unit 파일 헤더에만 기록된다.
...
## Planning Change Log 관리

스프린트 중 기획서 변경·구두 합의·회의 결정·개발 중 발견은 시시각각 들어온다. 이 사실을 워크플로우에 반영하기 위해 **기능 단위 변경 원장** `planning-change-log.md`를 둔다. 이 원장은 하나의 unit을 넘어 여러 큰 분류/unit에 영향을 줄 수 있으므로 **이 스킬(부모)이 중앙에서 관리**한다. unit 스킬은 자기 unit에 적용한 PC-ID만 기록한다.

### 위치

- 최상위 인덱스와 같은 디렉터리: `<source-document-directory>/planning-change-log.md`.
...
| ID | 날짜 | 출처 | 변경/합의 내용 | 영향 범위 | 반영 상태 | 연결 |
|----|------|------|----------------|-----------|-----------|------|
| PC-001 | 2026-07-14 | 구두 합의 / PM 홍길동 | 빈 결과는 404가 아니라 200+빈 배열 | API 응답, 테스트 | 반영 대기 | TR-003 |
```

- 출처 값: `Figma 변경` / `shopl-pm 정책서 변경` / `Jira 댓글` / `Slack·구두 합의` / `회의 결정` / `개발 중 발견`.
- 반영 상태: `기록됨` / `분석 필요` / `API draft 반영 필요` / `task-flow 반영 필요` / `테스트 반영 필요` / `구현 반영 완료` / `폐기`.
- 연결: 분해 문서의 Trace ID, 영향받는 큰 분류 / PC 적용 unit.

### 변경이 들어왔을 때 흐름

1. `planning-change-log.md`에 PC-ID로 기록(상태 `기록됨`).
2. 영향받는 Trace ID / 큰 분류 식별 후 상태를 `분석 필요`로.
3. API draft 영향 → `shopl-dev-api-draft` 수정 후 `API draft 반영 필요` 해소.
4. 진행 중 큰 분류/unit 영향 → 해당 큰 분류 `index.md`의 `Planning Changes` 표에 PC-ID 연결. `shopl-dev-task-flow-unit`은 unit 파일에 적용한 PC-ID를 기록.
5. 테스트 케이스 영향 → 해당 unit의 Test Case Review 재승인.
6. 구현 반영 후 PC-ID 상태를 `구현 반영 완료`로.
...
sequenceDiagram
    ...
    A->>D: Unit Index/공통 컴포넌트 정리
    A->>U: Orientation 제시(순서/공통 결정, 상세 계획 아님)
    U-->>A: 방향 확인 또는 수정 요청

    loop 각 작은 분류 (shopl-dev-task-flow-unit 위임)
        A->>D: unit 파일 생성(없으면)
        A->>C: 작은 분류 실행 위임 (unitFilePath 전달)

        C->>D: unit 파일에 상세 계획 작성
        C->>U: 상세 계획 승인 요청
        U-->>C: 승인 또는 수정 요청
        C->>C: 해당 작은 분류만 구현
        C->>U: diff 제시
...
2. **`진행 중` 작업이 있으면 해당 큰 분류 `index.md`의 Unit Index를 읽어 다음 `대기` unit을 식별한다.** 최상위 인덱스의 `최근 메모`는 상태 요약일 뿐 진실이 아니다. 다음 실행 unit은 오직 Unit Index `상태` 열 기준으로 결정하며, 메모와 불일치 시 Unit Index를 진실로 삼고 메모를 정정한다.
3. 현재 작업과 최근 완료/취소된 작업의 트래커 상태를 질의한다. (`jira` 모드만)
4. 계속 진행하기 전에 불일치를 조정한다:
   - `jira` 모드: 추적 상태와 Jira 불일치 → 매트릭스 따라 동기화.
   - `local` 모드: 불일치 없음 (추적 문서가 유일).
   - `linear` 모드: 현재 수동 확인.
5. 동기화 결과를 사용자에게 간략히 보고한다.
