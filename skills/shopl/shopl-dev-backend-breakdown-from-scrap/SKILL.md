...
description: "Scrap된 기획서, Figma 캡처, 와이어프레임, 샘플 Excel을 현재 백엔드 구현과 대조하여 기획 → 기술설계 사이의 백엔드 구현 분해 항목, 확인 필요 항목, 화면 단위 하위작업을 도출한다. 기획(입력) 산출물을 기술 설계로 넘길 백엔드 구현 단위로 쪼개는 브리지 역할. Trigger: scrap 기획서 분석, 백엔드 구현 분해, 백엔드 작업 분해, 기획-구현 gap 분석, Figma scrap 기반 백엔드 작업 도출."
---
...
1. **기획서 원문 전사 금지** — 현재 구현 확인 후 분류(구현 완료 / 일부 존재 / 데이터 모델만 / API 요청만 / 계산만 / Excel만 미구현 / 신규 필요 / 기획 확인 / 프론트 중심). "기능 설명"이 아니라 **현재 구현 대비 변경 필요사항** 중심으로 작성.
2. **화면 요소 ↔ 백엔드 구현 매핑** — Entity / Request·Response DTO / Mapper / Validator / Controller / Service 진입점 / Reader-Factory-Aggregator / Repository-Query / Excel drawer / Column adjustor / Message key / Error code / Migration 과 연결.
...
이 섹션은 반드시 존재해야 하고 비어 있으면 안 된다. 핵심 원칙 3번의 항목들을 한곳에 모은다. 추정한 내용은 "추정"임을 명시하고 근거를 표기하며, 사용자에게 직접 질문해야 할 사항을 포함한다.

## 개발설계 단계로의 인계

이 스킬의 산출물은 개발설계(시스템/API 설계) 단계의 입력이다.

- 공통 로직(2개 이상 화면 사용)은 Shared Service/Facade/Query Repository로 추출, 각 화면 Service가 주입받아 사용. 화면 전용 조립만 남기고 공통 로직 중복 구현 금지.
- **이 항목은 코드 아키텍처(공통 로직의 물리적 위치)에 관한 것**으로, 하위작업 분해(공통을 첫 API에 흡수)와 무관. 공통 로직은 흡수된 API 태스크가 Shared Service로 구현하되, 그 "구현" 작업을 첫 API가 맡는다.
