...
description: "Scrap된 기획서, Figma 캡처, 와이어프레임, 샘플 Excel을 현재 백엔드 구현과 대조하여 기획→기술설계 사이의 백엔드 구현 분해 항목, 확인 필요 항목, 화면 단위/공통 로직 단위 하위작업을 도출한다. 기획(입력) 산출물을 기술 설계로 넘길 백엔드 구현 단위로 쪼개는 브리지 역할. Trigger: scrap 기획서 분석, 백엔드 구현 분해, 백엔드 작업 분해, 기획-구현 gap 분석, Figma scrap 기반 백엔드 작업 도출."
---
...
이 스킬은 scrap과 현재 구현을 대조하는 스킬이므로 **둘 다 있어야 실행**.

- **필수 입력**: (a) scrap 자료 경로, (b) 구현 확인 범위(클래스/패키지/모듈/API 진입점/기능 흐름)
...
2. **화면 단위 분해** — 고유 화면(페이지/모달/탭/뷰) 단위로 화면 목록 작성, 여러 화면이 공유하는 집계·권한·추출 로직은 **공통 로직**으로 별도 식별. 형식은 `references/examples.md §1-2`.
...
8. **구현 하위작업 분해** — 큰 분류/작은 분류 2단계로 구성. 큰 분류는 하나의 API(또는 단일 도메인 컴포넌트) 단위를 넘지 않음. 작은 분류는 태스크 성격에 따라 다양함(분석/조사, DTO 설계, 도메인 설계, 테스트 케이스 논의, 테스트 코드 작성, 구현). **테스트가 의미 있는 비즈니스 로직 태스크**에는 `테스트 케이스 논의(사용자와) → 테스트 코드 작성 → 구현` 순서로 포함. DTO/조립/조사 중심 태스크처럼 검증 대상 로직이 크지 않은 경우에는 이 순서를 강제하지 않음. 형식은 `references/examples.md §7`.
...
- [ ] 화면 단위/공통 로직 단위 하위작업이 큰 분류/작은 분류 2단계로 구성, 큰 분류는 단일 API(또는 도메인 컴포넌트) 단위를 넘지 않음
