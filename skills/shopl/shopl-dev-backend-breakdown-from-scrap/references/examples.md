...
- 큰 분류는 하나의 API(또는 단일 도메인 컴포넌트) 단위를 넘지 않는다.
...
```md
### 큰 분류 1. 다운로드 기간 검증 로직 (도메인)

- 작은 분류
  - 도메인 설계: 다운로드 기간 제한 정책 모델
  - 테스트 케이스 논의: 기간 상한, 미래 날짜 차단 기준
  - 테스트 코드 작성: 미래 날짜 다운로드 차단
  - 구현: 기간 제한 검증 로직

### 큰 분류 2. 대상자 추출 로직 (도메인)

- 작은 분류
  - 분석: 기존 공통 대상자 후보 추출 로직 재사용 범위
  - 테스트 케이스 논의: 기록 존재 사용자 필터 기준
  - 테스트 코드 작성: 대상자 필터 회귀
  - 구현: 기록 존재 사용자 필터
...
### 큰 분류 4. 주간 요약 시트 생성 API

- 작은 분류
  - 요청/응답 DTO 설계: 주간 요약 응답 구조
  - 구현: 주 단위 기간 분할 로직
  - 구현: 주간 요약 시트 생성 로직
  - 구현: API 엔드포인트
```
