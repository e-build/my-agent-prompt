...
## 7. 구현 하위작업 분해 예시 (절차 8단계)

### 분류 기준

- 큰 분류는 하나의 API 단위를 넘지 않는다.
- **공통 로직은 별도 큰 분류로 분리하지 않는다.** 공통 로직은 그것을 처음 필요로 하는 API 태스크 안에 흡수하여 구현한다.
- 흡수된 공통 컴포넌트는 후속 API에서 재사용하므로, §15(구현 하위작업) 최상단에 **공통 컴포넌트 매핑표**를 둬서 누락을 방지한다.
- API 엔드포인트를 신규·변경하는 큰 분류의 **첫 작은 분류는 레이어별 스켈레톤 골격 클래스 생성**(빈 구현/시그니처만 먼저 만들고 이후 설계·테스트·구현 단계에서 채움). 엔드포인트를 만들지 않는 태스크(DTO-only/조사-only/도메인-only/Excel-only 등)는 골격 단계에서 제외.
- 큰 분류 안의 작은 분류는 태스크 성격에 따라 다양하다. 대표 유형:
  - [골격] 레이어별 스켈레톤 클래스 생성 (API 엔드포인트 큰 분류의 첫 단계)
  - 분석/조사
  - 요청/응답 DTO 설계
  - 도메인 모델/정책 설계 (흡수된 공통 로직 포함)
  - 테스트 케이스 논의 (사용자와)
  - 테스트 코드 작성
  - 구현 (로직/엔드포인트/조립)
- 테스트가 의미 있는 비즈니스 로직 태스크에는 `테스트 케이스 논의 → 테스트 코드 작성`을 구현 이전에 포함한다.
- DTO 설계/데이터 조립/조사 중심 태스크처럼 검증 대상 로직이 크지 않은 경우에는 이 순서를 강제하지 않는다.

### 공통 컴포넌트 매핑표 예시 (§15 최상단)

공통 로직이 하나 이상 있을 때 §15 최상단에 둔다. 어느 API에서 구현하고 어느 API에서 재사용하는지 한눈에 추적.

```md
| 공통 컴포넌트 | 구현 위치 (신규 생성 API) | 재사용하는 API 작업 |
|---|---|---|
...
- 구현 공통 컴포넌트 (이 작업에서 신규 생성)
  - 대상자 후보 추출 로직
  - 다운로드 기간 검증 로직
- 재사용 공통 컴포넌트: 없음 (최초 작업)
- 엔드포인트: `POST /admin/reports/weekly-summary`
- 작은 분류
  - [골격] 레이어별 클래스 생성 (빈 구현/시그니처): Controller · Request/Response DTO · Facade + Command · 도메인(대상자 추출, 기간 검증 + model/view) · Infrastructure(Repository/Client) — 표준 세트는 프로젝트 계층 구조 기준
  - [공통/대상자] 분석: 기존 대상자 후보 추출 로직 재사용 범위
  - [공통/대상자] 테스트 케이스 논의: 기록 존재 사용자 필터 기준
  - [공통/대상자] 테스트 코드 작성: 대상자 필터 회귀
  - [공통/대상자] 구현: 기록 존재 사용자 필터
  - [공통/기간검증] 도메인 설계: 다운로드 기간 제한 정책 모델
  - [공통/기간검증] 테스트 케이스 논의: 기간 상한, 미래 날짜 차단 기준
  - [공통/기간검증] 테스트 코드 작성: 미래 날짜 차단
  - [공통/기간검증] 구현: 기간 제한 검증 로직
  - [API] 요청/응답 DTO 설계: 주간 요약 응답 구조
  - [API] 구현: 주 단위 기간 분할 로직
  - [API] 구현: 주간 요약 시트 생성 로직
  - [API] 구현: API 엔드포인트
...
- 작은 분류
  - [골격] 레이어별 클래스 생성 (빈 구현/시그니처): Controller · Request/Response DTO · Facade + Command · 도메인(월간 집계 + model/view) · Infrastructure(필요 시)
  - [API] 요청/응답 DTO 설계: 월간 요약 응답 구조
  - [API] 구현: 월간 집계 로직
  - [API] 구현: API 엔드포인트
...
- 작은 분류
  - 도메인 설계: 휴무유형별 계산 결과 모델
  - 테스트 케이스 논의: 복수 휴무유형 동일일 집계 기준
  - 테스트 코드 작성: 휴무유형별 계산
  - 구현: 요약 시트 휴무유형별 동적 헤더
  - 구현: 상세 시트 휴무유형별 동적 헤더
```

> 참고: 큰 분류 3은 API 엔드포인트를 신규 생성하지 않으므로 [골격] 단계가 없다. 엔드포인트가 없는 도메인/Excel 전용 태스크는 골격 대상에서 제외된다.

### 흡수 방식의 핵심 포인트

- 공통 로직(대상자 추출, 기간 검증)이 별도 큰 분류가 아니라 **큰 분류 1의 작은 분류**로 들어간다.
- 큰 분류 2, 3은 해당 공통 로직을 `재사용 공통 컴포넌트`로 선언만 하고 다시 구현하지 않는다.
