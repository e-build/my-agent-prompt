...
## 의존성 (필수)
`es-query.sh`는 **bash + curl + jq** 세 가지를 필요로 한다 (단일 bash 셸 스크립트).

| 환경 | 준비 |
|------|------|
| macOS | `brew install jq` (curl은 기본 탑재) |
| Linux | `apt install jq` / `dnf install jq` (curl 보통 탑재) |
| **Windows** | **WSL2 권장** — 그 안에서 `apt install curl jq`. 순수 CMD/PowerShell은 미지원(이 스킬은 bash 스크립트). Git Bash는 bash+curl은 되나 jq는 별도 설치 필요 |

`curl` 또는 `jq`가 없으면 `es-query.sh`가 즉시 에러로 종료한다. jq는 JSON 검증·range 가드·`_source` 자동 삽입·결과 정제에 쓰이므로 생략 불가.
...
## ES 로그 한계 — 이런 질의는 DB로 가야 함
ES 로그는 "누가/언제/어떤 API를" 알려주지만, "무슨 값으로/복구 가능한지/현재 상태"는 모른다.
에이전트는 아래 질의를 받으면 빈 쿼리를 돌리지 말고 DB 조회를 먼저 안내할 것.

| 질의 유형 | ES 가능? | 근거 | 대체/보완 경로 |
|---|---|---|---|
| "무슨 값으로 변경됐는지" (전/후 값) | ❌ | ES엔 요청 파라미터/응답만, DB값 아님 | DB audit/history 테이블 |
| "복구 가능한지" | ❌ | soft delete 상태는 DB | DB `is_delete` 컬럼 |
| "현재 권한/상태 값" | ❌ | DB 현재값 | DB 조회 |
| "변경 시점 + 주체(누가 언제 API 호출)" | ✅ 보완 | rId/uId로 API 호출 시각 + 작업자 | ES로 시점·주체 보충 후 DB로 값 확인 |
| "삭제 누가/언제" | ✅ | `DELETE` requestURI + rId/uId | ES 1차, 복구여부는 DB |
| "삭제된 대상/기록이 무엇인지" | ❌ 보완 | DELETE body가 `{}`이고 도메인 로그가 없으면 ES에 대상 ID가 없음 | DB audit/history 또는 `is_delete` 변경 이력 |
| "로그인 실패 원인" | ✅ | Authentication/SecurityChain/OAuth2 | ES로 충분 |
...
```
> 대상 식별이 필요하면 `HTTP:RequestBody`에만 의존하지 말 것. body가 `{}`일 수 있으므로 같은 rId의 도메인 구조화 로그(`AttendanceRecord:*`, `LocationCheck:*` 등) `message_data`를 함께 본다.
...
### 7. 운영 액션 타임라인
특정 cId(고객사)에서 24시간 내 발생한 운영 액션(admin/managing/group/DELETE)을 시간순 추출. 이상 현상 직전 액션 추적.
```json
{
  "query": {"bool": {"filter": [
    ...
    {"term": {"env": "SHOPL"}},
    {"term": {"service_type": "Backend-API"}},
    {"terms": {"message_type": ["HTTP:RequestIn", "HTTP:ResponseOut"]}},
    {"term": {"cId": "<CLIENT_ID>"}},
    {"bool": {"should": [
      {"prefix": {"requestURI": "POST /admin/"}},
      {"prefix": {"requestURI": "PUT /admin/"}},
      {"prefix": {"requestURI": "DELETE "}},
      {"wildcard": {"requestURI": "*/managing*"}},
      {"wildcard": {"requestURI": "*/group*"}}
    ], "minimum_should_match": 1}}
  ]}},
  "sort": [{"@timestamp": "asc"}],
  "size": 100
...
### 8. 삭제 액션 추적
`DELETE` method로 특정 cId에서 삭제된 리소스 추적. 작업자(rId/uId)·시각 확보.
```json
{
  "query": {"bool": {"filter": [
  ...
  "sort": [{"@timestamp": "desc"}],
  "size": 50,
  "_source": ["@timestamp", "requestURI", "rId", "uId", "cId", "message_type"]
}
```
> ⚠ 복구 가능 여부는 ES로 판단 불가 → DB `is_delete`/soft delete 상태 확인 필요.

### 9. Shopl 계정 실제 작업자 역추적
운영자 계정이 DB에 `Shopl`로만 남아 실제 작업자를 알 수 없을 때, ES 로그로 역추적.
**연결고리는 2가지(둘 다 검증됨)**:
- **rId (주)**: IDP가 Feign으로 Backend-API 호출 시 `X-Request-ID` 헤더로 rId 전파 → 같은 rId가 IDP·Backend-API 양쪽에 존재. Recipe 8 등에서 확보한 운영 액션의 rId로 IDP 로그 역추적.
- **uId (보조)**: 크로스 service_type 공유 식별자. rId가 안 잡히면 uId + 시간 근접 필터.

**1단계 — rId로 IDP 인증 주체 역추적**:
```json
{
  "query": {"bool": {"filter": [
    {"term": {"rId": "<운영액션에서 확보한 rId>"}},
    {"term": {"service_type": "IDP"}},
    {"terms": {"message_type": ["OAuth2:TokenRequestCompleted", "OAuth2:TokenLifecycle", "Feign:RequestOut"]}}
  ]}},
  "sort": [{"@timestamp": "asc"}],
  "size": 20,
  "_source": ["@timestamp", "message_type", "message_data.userId", "message_data.accountId", "message_data.clientId", "rId"]
}
```
→ `OAuth2:TokenRequestCompleted.message_data.userId`/`accountId`로 실제 로그인 사용자 식별.

**2단계 (rId가 안 잡힐 때) — uId + 시간 근접**:
```json
{
  "query": {"bool": {"filter": [
    {"range": {"@timestamp": {"gte": "<운영액션시각-2m>", "lte": "<운영액션시각+2m>"}}},
    {"term": {"uId": "<운영액션 uId>"}},
    {"term": {"service_type": "IDP"}},
    {"term": {"message_type": "OAuth2:TokenRequestCompleted"}}
  ]}},
  "sort": [{"@timestamp": "asc"}],
  "size": 10,
  "_source": ["@timestamp", "message_data.userId", "message_data.accountId", "message_data.clientId", "rId"]
}
```

### 10. 로그인/인증 실패 원인 분석
`Authentication:AuthenticationFailed` + `SecurityChain:AccessDenied`로 원인 분류. `message_data.exceptionType`로 권한/토큰/기기 구분.
```json
{
  "query": {"bool": {"filter": [
    {"range": {"@timestamp": {"gte": "now-24h"}}},
    {"bool": {"should": [
      {"term": {"message_type": "Authentication:AuthenticationFailed"}},
      {"term": {"message_type": "SecurityChain:AccessDenied"}}
    ], "minimum_should_match": 1}}
  ]}},
  "sort": [{"@timestamp": "desc"}],
  "size": 50,
  "_source": ["@timestamp", "message_data", "requestURI", "rId", "uId"]
}
```
→ `exceptionType`(`InvalidBearerTokenException`=토큰 만료/무효 등) + `stack_trace`로 원인 세분화. IDP 활성화 누락 의심 시 `OAuth2:TokenLifecycle` 병행 조회.

### 11. 장기간 사용자 활동 요약
30일처럼 범위가 크면 원시 hit 조회 금지. 먼저 `HTTP:RequestIn`만 KST 일별 집계한다.
```json
{
  "size": 0,
  "query": {"bool": {"filter": [
    ...
    {"term": {"uId": "<USER_ID>"}},
    {"term": {"message_type": "HTTP:RequestIn"}}
  ]}},
  "aggs": {"by_day": {"date_histogram": {
    "field": "@timestamp", "calendar_interval": "day", "time_zone": "+09:00"
  }, "aggs": {
    "by_uri": {"terms": {"field": "requestURI", "size": 50}},
    "method": {"filters": {"filters": {
      "GET": {"prefix": {"requestURI": "GET "}},
      "POST": {"prefix": {"requestURI": "POST "}},
      "PUT": {"prefix": {"requestURI": "PUT "}},
      "DELETE": {"prefix": {"requestURI": "DELETE "}}
    }}}
  }}}
}
```
세부 대상은 `RequestBody`보다 도메인 로그(`AttendanceRecord:PunchIn/PunchOut` 등)로 보강한다.
...
- `terms.rId`(여러 rId)는 가능하면 시간 range를 함께 둔다. 다중 rId + 전체 기간 조회는 느려질 수 있다.
