...
| **shopl-dev** (개발) | `shopl-dev-` | 코드 구현/테스트/빌드/CI/배포 | auth-token, jira-task-flow, verify-feature, requirements-from-scrap |
| **shopl-work** (업무) | `shopl-work-` | Jira 조회/브리핑/문서/커뮤니케이션/행정 | jira-briefing, jira-direct |
| **shopl-fix** (트러블슈팅) | `shopl-fix-` | 디버깅/장애대응/성능분석/문제해결 | (향후 추가) |

## 원본 저장소

- **Shopl 애플리케이션 코드:** `shopl-server-sub/`, `shopl-authorization-server/`
- **스킬 원본 & 설정:** `e-build/my-agent-prompt/skills/`
