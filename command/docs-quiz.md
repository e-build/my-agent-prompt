...
**5문제씩 묶어서** `question` 도구 1회 호출로 출제합니다. `questions` 배열에 5개 질문을 담아 한 번에 표시합니다.

- 진행 상황 표시 후 `question` 도구 **1회 호출**에 5문제를 `questions` 배열로 묶어 출제
- 사용자가 5문제 모두 답변하면 다음 5문제 출제
- 마지막 묶음은 5문제 미만일 수 있음
- 모든 답변은 내부적으로 기록하여 Phase 5 채점에 사용
- **중간에 정답/오답을 절대 알려주지 않는다** — 정답 여부, 해설, 점수는 모든 문제 풀이가 끝난 뒤 Phase 5~6에서 한꺼번에 공개

**출제 예시** (Q1~Q5를 하나의 question 호출로):

```
📝 Q1~Q5 (5/{전체})
```

Use question (5개 질문을 하나의 호출로):
...
질문 2:
- header: "Q2 [주관식]"
- question: "Spring Boot에서 Virtual Thread를 활성화하는 설정 프로퍼티 이름은?\n\n직접 입력해주세요."
- options:
  - "힌트 보기" — spring.threads 관련 프로퍼티

질문 3:
- header: "Q3 [객관식]"
- question: "sealed interface의 주요 장점은?"
- options:
  - "①" — 런타임 성능이 향상된다
  - "②" — 컴파일 타임에 모든 하위 타입을 알 수 있어 when 분기가 완전해진다
  - "③" — 다중 상속이 가능해진다
  - "④" — 자동으로 data class가 된다

질문 4:
- header: "Q4 [서술형]"
- question: "learning/ 문서와 design/ 문서의 차이점을 설명하세요.\n\n2~5문장으로 서술해주세요."
- options:
  - "힌트 보기" — 범용성, 프로젝트 특화, 출처 필수 여부

질문 5:
- header: "Q5 [객관식]"
- question: "SSE 스트리밍에서 SseEmitter의 timeout을 0으로 설정하는 이유는?"
- options:
  - "①" — 즉시 연결을 종료하기 위해
  - "②" — 무제한 타임아웃으로 설정하기 위해
  - "③" — 기본값을 사용하기 위해
