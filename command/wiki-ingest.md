---
description: wiki에 새 소스를 추가하고 관련 위키 페이지를 생성/업데이트한다
---

# Wiki Ingest

새 소스를 wiki에 추가하는 워크플로우를 실행한다.
...
- `sources/` 하위에 없는 경우: 적절한 하위 디렉토리(`web/`, `memos/`, `work/`)를 판단해 복사한다.
  - 웹 기사/블로그 → `{wiki_root}/sources/web/YYYY-MM-DD-slug.md`
  - 직접 메모/회의록 → `{wiki_root}/sources/memos/YYYY-MM-DD-slug.md`
  - Slack/Jira/Confluence 추출 → `{wiki_root}/sources/work/YYYY-MM-DD-slug.md`
...
`{wiki_root}/wiki/summaries/YYYY-MM-DD-slug.md` 를 생성한다:
...
`{wiki_root}/wiki/index.md` 를 읽고, 새로 생성/업데이트한 모든 페이지를 해당 섹션 테이블에 추가/수정한다.
헤더의 날짜와 총 페이지 수도 업데이트한다.

### 8. log.md append

`{wiki_root}/wiki/log.md` 맨 아래에 아래 형식으로 기록을 추가한다:
