---
description: wiki 품질을 검사하고 모순, 고아 페이지, 누락 링크 등을 수정한다
---

# Wiki Lint

wiki의 건강 상태를 점검하고 품질 문제를 수정하는 워크플로우를 실행한다.
...
`{wiki_root}/wiki/index.md` 를 읽고, 실제 파일시스템도 확인해 index에 없는 페이지를 발견한다:
...
### 6. Git commit (config의 git: true인 경우, 수정이 있는 경우)
