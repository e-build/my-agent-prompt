...
- **정책서·기획서 재대조 의무화**: 테스트 케이스 표를 확정하기 전에, 이 unit이 커버하는 정책서/기획서의 관련 섹션을 반드시 다시 읽는다. 기존 기억이나 요약에 의존하지 않고 원본 문서를 직접 확인한다. 사용자가 정책서 경로를 명시한 경우 해당 파일을 반드시 읽는다. 정책의 분기/경계값/날짜 범위/권한/enum 가능값을 표의 "정책·요구사항" 열에 근거로 연결한다.
...
구현을 시작하기 전에 반드시 `git status --short`로 작업 디렉토리를 확인한다. **이 unit과 무관한 기존 변경분이 있으면 먼저 커밋한다.** 이전 작업의 변경분이 새 구현과 섞이는 것을 방지하기 위함이다.

- 기존 변경분이 있으면 사용자 승인 게이트 없이 즉시 커밋한다(커밋 접두사 규칙 준수). 이 커밋은 본 unit의 커밋이 아니라 "사전 정리" 커밋이다.
- 기존 변경분과 본 unit의 변경분을 한 커밋에 섞지 않는다.
- 작업 디렉토리가 깨끗하면 그대로 진행한다.
...
#### 통합 테스트 Mock/Stub 지양 원칙

Facade/Controller 통합 테스트에서는 **기본적으로 Mock/Stub을 사용하지 않는다**. 실제 DB fixture + 실제 Reader/Service 경로를 우선한다.

- 통합 테스트의 가치는 실제 파이프라인이 연결되어 동작하는지 검증하는 데 있다. Mock/Stub으로 의존성을 대체하면 그 검증이 사라진다.
- Mock/Stub이 불가피한 경우(외부 시스템 미구축, DB 스키마 제한, 비결정성 등)는 사유를 unit 파일의 Plan 또는 Execution Result에 명시하고 사용자 승인을 받는다.
- 도메인 순수 단위 테스트는 이 원칙의 대상이 아니다(의존성 주입으로 실제 객체 또는 test double 사용은 자유).

#### 커밋 단위 강제

사용자가 별도로 요청하지 않아도, 의미 있는 진행 단계마다 커밋을 남긴다. 한 번의 큰 커밋으로 unit 전체를 처리하지 않는다:

- RED(테스트 실패 확인) 후 커밋
- GREEN(구현 완료, 테스트 통과) 후 커밋
- 문서/검증 완료 후 커밋

각 커밋은 독립적으로 의미를 가져야 하며, 커밋 메시지로 어떤 단계인지 알 수 있어야 한다. 사용자가 명시적으로 커밋을 금지하지 않는 한 이 규칙을 강제한다.
...
7. **문서 일관성 감사(완료 전 필수)** — unit을 완료로 표시하기 전에 아래 항목을 실제 파일을 읽어 확인한다(기억이나 추정이 아닌 grep/read로 검증):
   - unit 파일 헤더 `상태:`가 `완료`로 설정됨
   - Unit Index(큰 분류 `index.md`)의 해당 행 `상태`/`커밋`이 갱신됨
   - 테스트 케이스가 의미 있는 unit은 Test Case Review의 모든 TC-ID가 Execution/Verification Result와 일치함
   - 미사용/중복/빈 초안 unit 파일이 잔재하지 않음
   - (TODO 도구를 사용 중인 경우) 해당 TODO 항목 상태가 unit 상태와 일치함
