...
- 문항 수 기준 비중은 아래를 기본값으로 한다.
  ...
  - 서술형: 약 10% (`essay`)
- 10문항 기준 고정 예시는 **객관식 7문항 + 단답형 주관식 2문항 + 서술형 1문항**이다.
- 10문항을 초과하면 위 비율에 가장 가깝게 배분한다. 단, 사전진단은 빠른 진단이 목적이므로 서술형을 과도하게 늘리지 않는다.
- 총점은 보통 100점으로 두되, 문항 수 비중과 별개로 난이도에 따라 배점한다.

### 문항 타입 구분

- `short-answer`는 짧은 서술형이 아니라 **단답형 주관식**이다.
  - 정확한 키워드/용어/값/명령어/헤더명/테이블명/규칙명 1~2개로 답할 수 있어야 한다.
  - 답변은 보통 1~5단어 또는 한 줄 이내여야 한다.
  - 예: `ETag`, `flyway_schema_history`, `leftmost prefix`, `Cache-Control`, `TTL`.
  - `short-answer`에서 "왜", "설명하시오", "비교하시오", "판단하시오"처럼 문장형 설명을 요구하지 않는다. 그런 문제는 `essay`로 분류한다.
- `essay`는 관계, 이유, 판단 근거, 흐름, 트레이드오프를 자기 말로 설명하는 서술형이다.
- `code`/`sql`을 주관식 비중 안에서 사용할 때는 **짧은 정답형**이어야 한다.
  - 예: 한 줄 명령, 짧은 WHERE 조건, 함수명, 짧은 SQL 조각.
  - 여러 줄 코드 작성, 설계 판단, 원인 분석은 `essay`가 아니라 lab/test로 넘긴다.

### 기술스택 기반 코드 보기 문항

학습 설계 브리프나 챕터 README에서 기술스택/도구/언어/DB가 정해져 있으면, 진단 문항에 코드/SQL/config/log 조각을 보기 또는 상황으로 제공할 수 있다.

- 목적은 코드를 길게 작성시키는 것이 아니라, **주어진 조각을 읽고 개념을 식별하거나 결과를 판단하게 하는 것**이다.
- 객관식 문항에서는 코드/SQL/config 조각을 `description`에 넣고, 선택지에서 동작 결과·문제 원인·올바른 수정 방향을 고르게 한다.
- 단답형 주관식 문항에서는 코드/SQL/config 조각을 보고 정확한 키워드/함수명/헤더명/테이블명/설정값 1~2개를 답하게 한다.
- 코드 보기 문항은 기술스택이 명확할 때만 사용한다. 기술스택이 불명확하면 일반 개념/용어 진단을 우선한다.
- 보기 코드는 짧고 self-contained해야 한다. 진단 문항 하나가 긴 lab 과제가 되면 안 된다.
...
```json
{
  ...
  "type": "single-choice",
  "sectionId": "apply",
  "prompt": "아래 Spring Cache 코드에서 캐시 키에 포함되는 값은?",
  "description": "```java\n@Cacheable(value = \"users\", key = \"#userId\")\npublic User getUser(Long userId) { ... }\n```",
  "options": [
    { "id": "A", "text": "userId" },
    { "id": "B", "text": "users 테이블 전체" },
    { "id": "C", "text": "User 클래스 이름" },
    { "id": "D", "text": "메서드 반환값" }
  ],
  "rubric": ["짧은 코드 조각에서 캐시 키 표현식을 읽는다"]
}
```
...
### 예시 JSON (10문항: 객관식 7 + 단답형 주관식 2 + 서술형 1)
...
```json
{
  ...
  "questions": [
    ...
    },
    {
      ...
      "type": "short-answer",
      "sectionId": "explain",
      "prompt": "복합 인덱스 `(a, b, c)`에서 왼쪽부터 연속된 조건만 효율적으로 쓰는 규칙 이름은?",
      "points": 12,
      "placeholder": "예: leftmost prefix",
      "rubric": ["leftmost prefix 또는 왼쪽 접두사 규칙을 답한다"]
    },
    {
      "id": "q9",
      "type": "short-answer",
      "sectionId": "explain",
      "prompt": "MySQL 실행 계획에서 실제 사용 인덱스를 보여주는 필드명은?",
      "points": 12,
      "placeholder": "필드명 1개",
      "rubric": ["key를 답한다", "대소문자 차이는 허용한다"]
    },
    {
      "id": "q10",
      "type": "essay",
...
- 단답형 주관식(`short-answer`)은 정확한 키워드/용어/값을 기억에서 꺼내게 한다.
- 서술형(`essay`)은 관계/이유/판단을 자기 말로 설명하게 한다.
- 기술스택이 정해져 있으면 짧은 코드/SQL/config/log 조각을 보기로 주고, 그 조각에서 개념을 식별하거나 결과를 판단하게 한다.
...
7. tool 호출이 실패하거나 extension이 없으면, 그 사실을 학습자에게 알리고 `/reload` 또는 `bash scripts/install-pi --restore` 를 안내한다. 수동 fallback(직접 파일 열기/복사)은 제공하지 않는다 — diagnosis는 extension 기반이 표준이다.
