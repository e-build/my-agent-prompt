...
argument-hint: "<학습주제>"
---
<!-- Args: $ARGUMENTS = <주제 전체> (required, e.g. "영어 회화", "재무회계", "Redis 7", "백엔드 애플리케이션에서의 캐싱") -->
# 학습 프로젝트 초기화

학습 주제: **${ARGUMENTS:-주제를 지정하세요}**

`학습 주제`는 반드시 `$ARGUMENTS` 전체를 사용한다. `$1`만 사용하지 않는다. 예: `/study-init 백엔드 애플리케이션에서의 캐싱`이면 주제는 `백엔드`가 아니라 `백엔드 애플리케이션에서의 캐싱`이다.
만약 학습 주제가 `주제를 지정하세요`로 남아 있으면 프로젝트를 만들지 말고 사용자에게 `/study-init <학습주제>` 형식으로 다시 실행하라고 안내한다.
...
- 도구의 질문 개수 제한 때문에 별도 5번째 기술 질문을 만들지 않는다. 선호 챕터 규모는 3번 질문에, 기술/개발/데이터/도구/프레임워크/인프라 주제의 실습 스택/환경 제약은 4번 질문에 함께 받는다.
...
3. 범위/규모: 꼭 포함할 것과 제외하거나 가볍게 다룰 것이 있나요? 선호하는 챕터 규모가 있으면 함께 알려주세요. 예: 짧게(3~5), 표준(8~12), 깊게(15+).
...
1. 목적/성공 기준:
2. 현재 수준:
3. 범위/규모(포함/제외 + 선호 챕터 규모):
4. 참고 출처/환경(기술 주제라면 스택/버전/로컬 제약 포함):
```

묻지 않는 것:
- 헷갈리는 개념: `/study-chapter diagnosis`가 확인한다.
- 정확한 챕터 수 확정: 사용자의 선호 규모는 받되, 최종 챕터 수와 순서는 대표 자료의 학습 순서와 개념 의존성을 조사한 뒤 조정한다.
...
실행: /reload 후 현재 학습 프로젝트에 대해 학습 설계 미리보기를 다시 열어달라고 요청하세요. 그래도 없으면 bash scripts/install-pi --restore 후 /reload 하세요.
```
...
**전제 조건**: `study` extension이 설치되어 있어야 한다. `bash scripts/install-pi --restore` 후 `/reload`. extension이 없으면 diagnosis 단계는 동작하지 않는다(수동 fallback도 제공하지 않는다 — 사전진단은 extension 기반이 표준이다).
