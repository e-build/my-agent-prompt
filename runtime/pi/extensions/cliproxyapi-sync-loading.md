...
`runtime/pi/extensions/cliproxyapi-sync.ts`가 pi에 로드되고 동작하는 전체 흐름을 설명한다.
...
sequenceDiagram
    ...
    Note over pi,FS: 탐색 순서:<br/>~/.pi/agent/extensions/*.ts<br/>.runtime/pi/extensions/*.ts<br/>settings.json extensions[]<br/>-e 플래그 경로
